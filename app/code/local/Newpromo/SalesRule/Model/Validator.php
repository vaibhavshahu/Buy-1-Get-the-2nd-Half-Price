...
class Newpromo_SalesRule_Model_Validator extends Mage_SalesRule_Model_Validator
{
    ...
    // new member variable made for this promotion rule
    public $getproprice = array();
    public $getproqty = array();
    public $previous_discount = 0;
        ...
        foreach ($this->_getRules() as $rule) {
            ...
            switch ($rule->getSimpleAction()) {
                ...
                case Mage_SalesRule_Model_Rule::BUY_X_GET_Y_ACTION:
                 ...
                 case Newpromo_SalesRule_Model_Rule::BUY_1_GET_THE_2ND_HALF_PRICE_ACTION:
                    $this->getproprice[]=$itemPrice;
                    $this->getproqty[]=$qty;                                       
                    /*new code */
                    $priceArr = array();
                    foreach($this->getproprice as $key=>$price){
                        $discount=array();    
                        $qty = $this->getproqty[$key];
                        for ($i=0; $i < $qty; $i++) {
                            $priceArr[]=$price;
                        }
                        $qty_sum=count($priceArr);
                        asort($priceArr);
                        $arrFinal=array_values($priceArr); 
                    
                        $numMin=floor($qty_sum/2);
                        for($i=0;$i<$numMin;$i++){
                            $discount[]=$arrFinal[$i];
                        }
                        
                        $finalDiscount=array_sum($discount)/2;
                        
                    }
                    $applied_qty = $finalDiscount - $this->previous_discount;
                    
                    $this->previous_discount = $finalDiscount;
                    /* new code */
